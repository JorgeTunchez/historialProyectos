...
function getMenu(){
  $arrMenu = array();
  $strQuery = "SELECT mn.nombre nombre_menu, 
                      lower(sm.nombre) nombre_submenu
                 FROM submenu sm
                      INNER JOIN menu mn on sm.menu = mn.id 
             ORDER BY mn.nombre, sm.nombre";
  $result = executeQuery($strQuery);
  if (!empty($result)) {
    while ($row = mysqli_fetch_assoc($result)) {
        if (isset($arrMenu[$row["nombre_menu"]])){
          $arrMenu[$row["nombre_menu"]][$row["nombre_submenu"]]= $row["nombre_submenu"];
        }else{
          $arrMenu[$row["nombre_menu"]] = array();
          $arrMenu[$row["nombre_menu"]][$row["nombre_submenu"]]= $row["nombre_submenu"];
        }
  ...
  ?>
    ...
    <html lang="en">
      <head>
        ...
        <title>Historial Proyectos</title>
...
function drawHeader($strUsuario){
  ?>
  <!-- ======= Header ======= -->
      ...
      <ul class="d-flex align-items-center">
          ...
          <li class="nav-item dropdown pe-3">
            <a class="nav-link nav-profile d-flex align-items-center pe-0" href="#" data-bs-toggle="dropdown">
                ...
                <span class="d-none d-md-block dropdown-toggle ps-2"><?php print $strUsuario; ?></span>
            ...
            <ul class="dropdown-menu dropdown-menu-end dropdown-menu-arrow profile">
                ...
                <li><hr class="dropdown-divider"></li>
                <li>
                  <a class="dropdown-item d-flex align-items-center" onclick="destroSession()">
...
function drawMenu($strNamePage = "", $intMenu = 0){
  ?>
  <!-- ======= Sidebar ======= -->
    ...
    <ul class="sidebar-nav" id="sidebar-nav">
      ...
      <li class="nav-item">
        ...
        foreach( $arrMenu as $key => $val ){
          $menu = $key;
          ?>
          <a class="nav-link collapsed" data-bs-target="#<?php print $menu; ?>-nav" data-bs-toggle="collapse" href="#">
            <i class="bi bi-journal-text"></i><span><?php print $menu; ?></span><i class="bi bi-chevron-down ms-auto"></i>
          </a>
          <ul id="<?php print $menu; ?>-nav" class="nav-content collapse " data-bs-parent="#sidebar-nav">
            <?php
            foreach( $val as $key => $val2 ){
              $submenu = $key;
              ?>
              <li><a href="<?php print $submenu.'.php'; ?>"><i class="bi bi-circle"></i><span><?php print ucwords($submenu); ?></span></a></li>
              <?php
            }
            ?>
          </ul>
